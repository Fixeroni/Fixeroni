...
export const Route = createFileRoute("/artisan/dashboard/DashBoardComponent/SideBarNav")({
 ...
 function SideBarNav() {
    const menuItems = useMemo(
        () => [
          ...
          { name: "Messages", icon: "/images/icons/messageIcon.png", notification: 5 },
                    ...
                    {menuItems.map(({name, notification, icon})=>{
                        return(
                            ...
                            <div className=" flex flex-col items-center">
                                ...
                                {notification && (
                                          <span className="bg-[#ee3636e5]  mt-[-1rem]
                                           w-fit h-[19px] text-white rounded-[3px] 
                                          flex items-center justify-center px-[5px] py-[5px]">{notification}</span>
                                )}
                                </div>
                        
                               <div>
                               <h2 className={` ${name !== "Dashboard" ? "text-[#1E1E1E] opacity-[0.5]": "text-[#0F9067]"}  font-medium text-[1.2rem] poppins`}>{name}</h2>
                               </div>
                        ...
                        {settingMenu.map(({name, icon})=>{
                                return(
                                    ...
                                    <div className=" flex flex-col items-center">
                                           ...
                                           <div>
                                           <h2 className={` text-[#1E1E1E] opacity-[0.5]  font-medium text-[1.2rem] poppins`}>{name}</h2>
                                           </div>
                                           </div>
                                    </li>
                                )
